...
> 狀態：設計稿（2026-07-07）。**Phase 1 已實作並雲端/桌面雙模式全支援**（ADR-004,
> 2026-07-10）,本檔現在是設計理由與 UI 佈局的紀錄,不是待辦清單;現況見
> docs/ARCHITECTURE.md §5、§6,格式契約見 docs/specs/character-pack-format.md。
...
```
┌ 角色工房 ────────────────────────────────────────────┐
│ ① 我的角色包（2026-07-11 改勾選制：可同時勾多包）        │
│    [內建 Yolia]（標記「內建」，恆為基底，無操作鈕）       │
│    [✓啟用] 幽靈小妹 v1.0.0 by xxx   (編輯)(匯出)(刪除)   │
│    （擴充包可複選；換角包一次只能勾一個，勾新的自動取消舊的）│
...
7. 詢問「立即啟用？」→ 是則把 id 加進 `config.activePackIds`（勾選制）。
