...
「分類」欄的值對應 demo 頁工具列「顯示：」跟搜尋視窗「特殊類型」用的同一套三分類（`chat`/`donation`/`system`，中文字樣直接照抄畫面上的下拉選單），來源是 [public/labels.js](public/labels.js) 的 `EVENT_TYPE_LABELS[type].category`——挑「一般聊天」還是「抖內/贊助」還是「系統事件」，決定了這則訊息在 demo 頁篩選時會被歸進哪一類。標「—」的是附加在其他事件上的欄位（例如使用者名稱顏色、表情符號圖片渲染），本身不是獨立 `event_type`，沒有自己的分類。

| 平台 | 事件 | `event_type` | 分類 | 原始封包/事件 | 狀態 | 說明 |
|---|---|---|---|---|---|---|
| YouTube | 一般聊天 | `chat` | 一般聊天 | `liveChatTextMessageRenderer` | ✅ 驗證過 | 多次收到真實訊息 |
| YouTube | Super Chat（文字） | `superchat` | 抖內/贊助 | `liveChatPaidMessageRenderer` | ✅ 驗證過 | 真實樣本確認 `amount`/`color` 欄位；跟 `isMembership` 可能同時為真時目前只走 `superchat` 分支，會籍資訊會遺失（已知落差，見 `docs/event-types.md`） |
| YouTube | Super Sticker（貼圖） | `supersticker` | 抖內/贊助 | `liveChatPaidMessageRenderer`（`sticker` 有值） | ⚠️ 沒驗證過 | 還沒抓到真實樣本；2026-08-15 之前跟 `superchat` 共用同一個 raw capture tag，加 `youtube:superchat` 進 skip 會連這個一起濾掉——已經拆成獨立的 `youtube:supersticker` tag，現在可以只 skip 文字版、繼續收貼圖版 |
| YouTube | 會籍留言 | `chat`（`extra.isMembership`） | 一般聊天 | `liveChatMembershipItemRenderer` | ✅ 驗證過 | 129 則真實會籍留言，欄位正確 |
| YouTube | 會籍月數 | `chat`（`extra.membershipMonths`） | 一般聊天 | `liveChatMembershipItemRenderer`（`authorBadges`） | ✅ 驗證過 | 237 筆真實驗證，含「New member」/「Member (N months)」/「Member (N year(s))」三種格式；`0` 個月（`badge.label === "New member"`）就是新加入的判斷方式，不是獨立事件，是這個人第一則帶會籍徽章的聊天訊息 |
| YouTube | 新加入會籍系統通知 | `chat`（`extra.membershipHeader`） | 一般聊天 | `liveChatMembershipItemRenderer`（`headerSubtext`） | ⚠️ 沒驗證過 | 「剛加入會籍」這件事本身沒有收到過獨立的系統通知——累積 300+ 筆真實會籍留言（237+64，2026-08-15 再次確認），`headerSubtext` 全部是空的；能抓到的只有「會籍月數」列講的「第一則帶徽章的聊天」，不是系統主動推播 |
| YouTube | 里程碑系統通知 | `chat`（`extra.membershipHeader`） | 一般聊天 | `liveChatMembershipItemRenderer`（`headerSubtext`） | ⚠️ 沒驗證過 | 續訂到特定月數時 YouTube 理論上會給一次帶提示文字的系統通知機會，但同一批 300+ 筆真實樣本裡一次都沒遇過，`headerSubtext` 全部是空的，沒辦法確認格式長怎樣 |
...
| YouTube | 贈送會籍(購買方) | `membership_gift` | 抖內/贊助 | `liveChatSponsorshipsGiftPurchaseAnnouncementRenderer` | ✅ 驗證過 | 2026-08-15 真實封包驗證，`amount` = 份數，`extra.planName` = 會籍方案名稱（截圖對照真實畫面確認過文字意思）；不是 `ChatItem` 欄位，繞開套件另外處理原始 action，見 `connectors/youtube.js` 的 `patchYoutubeParser()` |
| YouTube | 贈送會籍(領取方) | `membership_gift_received` | 抖內/贊助 | `liveChatSponsorshipsGiftRedemptionAnnouncementRenderer` | ✅ 驗證過 | 同上，只有一筆真實樣本，贈送者名字用「message.runs 最後一個 run」抓，還沒有多筆樣本驗證這個位置一定固定 |
| YouTube | 聊天表情符號圖片渲染 | `chat`（`extra.messageParts`） | — | `liveChatTextMessageRenderer`（`message.runs`） | ✅ 驗證過 | 真實 `:face-blue-smiling:` 樣本驗證過 |
| Twitch | 一般聊天 | `chat` | 一般聊天 | `PRIVMSG` | ✅ 驗證過 | 多次收到真實訊息 |
| Twitch | 頻道點數兌換（醒目留言，有文字） | `chat_highlight` | 一般聊天 | `PRIVMSG`（`custom-reward-id` tag） | ⚠️ 沒驗證過 | 跟一般 `PRIVMSG` 走同一路徑理論上沒問題，但沒有專門拿真實兌換訊息驗證過 |
| Twitch | Bits 抖內 | `cheer` | 抖內/贊助 | `PRIVMSG`（`bits` tag） | ⚠️ 沒驗證過 | 沒有實際看過一筆真的 cheer 事件 |
| Twitch | 新訂閱 | `sub` | 抖內/贊助 | `USERNOTICE`（`msg-id=sub`） | ✅ 驗證過 | 抓到真實封包（Prime/付費 Tier 1），2026-08-15 使用者對照 demo 頁跟 Twitch 原文抓到「預先訂閱多個月」漏掉月數的落差並修正（`msg-param-multimonth-duration`） |
| Twitch | 續訂月數 | `resub` | 抖內/贊助 | `USERNOTICE`（`msg-id=resub`） | ✅ 驗證過 | 抓到真實封包，`cumulative-months` 欄位正確；2026-08-15 補上「續訂順便預先付多個月」（`msg-param-multimonth-duration`，跟 `sub` 同一個 tag/同一種落差） |
| Twitch | 贈送訂閱 | `subgift`/`submysterygift` | 抖內/贊助 | `USERNOTICE`（`msg-id=subgift`/`submysterygift`） | ⚠️ 部分驗證 | 抓到真實連續贈送封包，沒在 demo 頁看過渲染結果 |
| Twitch | Raid | `raid` | 系統事件 | `USERNOTICE`（`msg-id=raid`，`tmi.js` 轉成 `raided` 事件） | ⚠️ 沒驗證過 | 沒有實際看過一筆真的 raid 事件 |
| Twitch | 公告 | `announcement` | 系統事件 | `USERNOTICE`（`msg-id=announcement`） | ⚠️ 部分驗證 | 抓到真實公告封包，證實是原生 `/announce`，沒在 demo 頁看過渲染結果 |
| Twitch | 其他系統通知 | `usernotice_other` | 系統事件 | `USERNOTICE`（其他 `msg-id`） | ⚠️ 部分驗證 | 目前只真實遇過 `viewermilestone`（連續觀看場次里程碑）這個子類型 |
...
| SOOP | 一般聊天 | `chat` | 一般聊天 | `soop-extension` `CHAT`（底層代碼未查證） | ✅ 驗證過 | 大量真實訊息 |
| SOOP | 系統通知 | `notification` | 系統事件 | `soop-extension` `NOTIFICATION`（底層代碼未查證） | ✅ 驗證過 | 2026-08-15 抓到真實通知（BJ 自訂的贈禮項目說明），`res.notification` 是字串，欄位對應正確，demo 頁顯示正常 |
| SOOP | 表情訊息 | `emoticon` | 一般聊天 | `soop-extension` `EMOTICON`（底層代碼未查證） | ⚠️ 部分驗證 | `emoticonId` 是 OGQ 雜湊 ID，換不出圖片網址；圖片渲染缺口追蹤在 GitHub Issues |
| SOOP | 訂閱月數 | `subscribe` | 抖內/贊助 | 協定代碼 `0093`（`FOLLOW_ITEM_EFFECT`，已對照 soopapi） | ⚠️ 沒驗證過 | 可能只在續訂觸發，新訂閱疑似另一協定類型（`0091`／`FOLLOW_ITEM`，已抓到真實樣本但還沒接成事件）；追蹤在 GitHub Issues |
| SOOP | 文字/語音抖內 | `text_donation` | 抖內/贊助 | `soop-extension` `TEXT_DONATION`（底層代碼未查證） | ✅ 部分驗證 | 抓到真實連續送星球封包（27 顆同一人），沒在 demo 頁看過渲染結果 |
| SOOP | 廣告氣球抖內 | `ad_balloon_donation` | 抖內/贊助 | `soop-extension` `AD_BALLOON_DONATION`（底層代碼未查證） | ✅ 部分驗證 | 抓到真實封包，沒在 demo 頁看過渲染結果 |
| SOOP | 影片抖內 | `video_donation` | 抖內/贊助 | `soop-extension` `VIDEO_DONATION`（底層代碼未查證） | ❌ 完全沒測過 | 欄位結構跟已驗證的其他抖內管道相同，這個管道本身還沒實測 |
...
| SOOP | 贈送禮物 | `gift_item` | 抖內/贊助 | 協定代碼 `0045`（`SEND_QUICK_VIEW`，自己反推接上，非 `soop-extension` 支援） | ✅ 部分驗證 | 送禮者/收禮者暱稱對照真實截圖確認正確；`itemType` 對應表未知，追蹤在 GitHub Issues |
...
Issues 找，避免同一件事要維護兩個地方。目前追蹤中的項目大致有：YouTube 一般會籍留言分不出
「新加入/連續/贈禮」是哪一種（贈送會籍這個動作本身已經另外接上，見上面 `membership_gift`/
`membership_gift_received`，這裡剩下的是「已經有會籍在聊天的人，事後回頭看是不是被贈送的」分不出來）、
YouTube superchat+會籍同時為真時的顯示落差、SOOP 官方 API 模式、SOOP `gift_item` 的
