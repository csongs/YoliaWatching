...
`/응원3_s/`、`/축하해_s/`、`/하트뿜뿜/` 這種）——2026-08-14 使用者截圖確認 `/하트뿜뿜/` 在真正
的 SOOP 網頁上顯示的是**動畫（GIF）**，不是靜態圖，這批應該是完全獨立的「動態表情符號」目錄，
不只是網址不同，連檔案格式都不同。先後試了十幾個合理猜測的路徑（`gif`/`ani`/`motion`/
`small` 等常見命名慣例的排列組合）都是 404，沒有再繼續亂試——已經超出「合理猜測」的範圍，
再猜下去報酬遞減，且對第三方伺服器送太多探測請求不禮貌；比較務實的下一步是使用者從瀏覽器
開發者工具的「網路」分頁找這批動態表情符號實際載入的網址（有畫面在跳出來一定有對應的網路
請求）。已經接上 `connectors/soop.js`（見下面）的是**經典靜態圖那 123 筆**，連線時背景抓一次、
